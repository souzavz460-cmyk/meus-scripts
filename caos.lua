...
safeButton(ConfigTab, "DESTRUIR TUDO", function()
    ...
    infJump = false; antiLive = false; stealthSpeed = false
...
local function aimbotStep()
    ...
    local alpha = 0.02
    if aimForce == 1 then alpha = 0.02
    elseif aimForce == 2 then alpha = 0.14
    elseif aimForce == 3 then alpha = 0.45
    elseif aimForce == 4 then alpha = 0.75
    elseif aimForce == 5 then alpha = 1.0 end
    ...
    if bypass > 1 then
        ...
        if bypass == 10 then
            ...
            if aimForce < 5 then alpha = math.clamp(alpha * (1 - safetyFactor * 0.3), 0.01, 0.5) end
        else
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        ...
        if espBox and useDrawing then
            ...
            if top and bot then
                ...
                if not boxes[p] then pcall(function() boxes[p] = Drawing.new("Square"); boxes[p].Thickness = 2; boxes[p].Filled = false end) end
                if boxes[p] then boxes[p].Position = Vector2.new(cx - w/2, top.Y); boxes[p].Size = Vector2.new(w, h); boxes[p].Color = boxColor; boxes[p].Visible = true end
            end
        end
        ...
        if espSkel and useDrawing then
            if not skeletons[p] then
                ...
                for i, pair in ipairs(bonePairs) do
                    ...
                    if a and b then pcall(function() local line = Drawing.new("Line"); line.Thickness = 1; skeletons[p][i] = {line = line, a = a, b = b} end) end
                end
            end
            for _, data in ipairs(skeletons[p]) do
                ...
                if aVis and bVis then data.line.From = Vector2.new(aPos.X, aPos.Y); data.line.To = Vector2.new(bPos.X, bPos.Y); data.line.Color = skelColor; data.line.Visible = true
        ...
        if showNameHealth and useDrawing then
            ...
            if on then
                local moneyStr = ""
                if showMoney then local money = getPlayerMoney(p) if money then moneyStr = " $" .. money end end
                local text = p.Name .. " [" .. math.floor(health) .. "/" .. math.floor(maxHealth) .. "]" .. moneyStr
                if not nameTags[p] then pcall(function() nameTags[p] = Drawing.new("Text"); nameTags[p].Center = true; nameTags[p].Size = 14; nameTags[p].Outline = true; nameTags[p].OutlineColor = Color3.new(0,0,0) end) end
                if nameTags[p] then nameTags[p].Text = text; nameTags[p].Position = Vector2.new(headPos.X, headPos.Y - 10); nameTags[p].Color = Color3.new(1,1,1); nameTags[p].Visible = true end
            end
        end
    end

    if fovCircleObj then
        fovCircleObj.Position = Camera.ViewportSize / 2; fovCircleObj.Radius = fovRadius; fovCircleObj.Visible = fovCircle
