-- [[ SZ MODS V4 – Edição Avançada & Key System ]] --
...
-- // CONFIGURAÇÃO DO ENDPOINT DA SUA KEY (Altere para o link do seu site/raw onde ficam as chaves válidas)

local URL_DAS_KEYS = "https://sua-url-aqui.com/keys.txt"
...
-- // Inicialização da Janela UI com Key System Ativado
...
local Window = Rayfield:CreateWindow({
    ...
    KeySystem = true, -- Ativa a verificação de Key

    KeySettings = {

        Title = "SZ SYSTEM | Autenticação",

        Subtitle = "Insira sua Chave de Acesso",

        Note = "Gere chaves de 1, 3, 30 ou 1000 dias no painel do Souza.",

        FileName = "SZ_System_Key", 

        SaveKey = true, 

        GrabKeyFromUrl = true, 

        Key = { URL_DAS_KEYS } -- O Rayfield vai ler as chaves válidas direto daqui
...
if useDrawing then
    ...
    pcall(function()
        ...
        drawingCache.fovCircleObj.Filled = false -- Correção definitiva do fundo
...
-- // Pipeline de ESP (Universal R6 e R15)
...
local function renderESPPipeline()
    ...
    -- Círculo do FOV Corrigido (Garante Filled = false em tempo de execução)
    ...
    if drawingCache.fovCircleObj then
        ...
        drawingCache.fovCircleObj.Filled = false -- Sem preenchimento de fundo
...
-- // Loop Unificado de Física e Proteção
...
RunService.RenderStepped:Connect(function(deltaTime)
    ...
    -- Velocidade Disfarçada por CFrame
