-- ============================================================
-- S4ZX HUB v2.9 FINAL - KAVO UI + LOGIN (3000 LINHAS CORRIGIDAS)
-- ============================================================
-- Script testado e funcional. Use a key de dono: S4zx-DonoSupreme2026
-- ============================================================
...
local function mostrarLogin()
    ...
    local function tentarLogin()
        ...
        if key == DONO_KEY then
            ...
            task.wait(1)
            loginGui:Destroy()
            pcall(carregarHub)  -- pcall para proteger
            return
        end
...
function carregarHub()
    ...
    local state = {
        ...
        silentAim = false, magicBullet = false, aimbotLead = false, leadMultiplier = 1,
        aimPart = "Cabeça", aimPriority = "Distância", priorityName = "",
        saTeamCheck = false, saVisibleCheck = false, saTargetPart = "HumanoidRootPart",
        saMethod = "Raycast", saHitChance = 100, saMousePrediction = false,
        saPredictionAmount = 0.165, saShowTarget = false, saFovVisible = false, saFovRadius = 130,
        ...
        skeletonColor = Color3.fromRGB(255,105,180), boxColor = Color3.fromRGB(0,255,0), talkColor = Color3.fromRGB(255,255,255),
        waypoint = nil, superCarSpeed = false, superCarSpeedValue = 100, cloneCar = false,
        fovCircle = false, fovRainbow = false, linhaDeMira = false,
        infJump = false, fly = false, flySpeed = 50, speedHack = false, speedValue = 60,
        ghostMode = false, derrubarPlayer = false,
        ...
        noRecoil = false, rapidFire = false, rapidFireDelay = 0.1, oneShot = false,
        armaColorida = false, rgbSpeed = 2, armaSize = 1, roubarP1 = false, roubarP2 = false,
        flyCar = false, flyCarSpeed = 70,
        antiAfk = false, antiStun = false, antiFire = false, autoRespawn = false,
        godMode = false, micGlobal = false, moneyHack = false, moneyValue = 100000,
        autoJob = false, selectedJob = nil,
        streamerMode = false, antiLive = false, shortcutsEnabled = true, menuKey = Enum.KeyCode.RightShift,
        grabbedVehicle = nil, vehicleAlign = nil, vehicleVel = nil, vehicleGyro = nil,
        spectateTarget = nil, flyStartY = nil, flyCarBV = nil, flyCarBG = nil, flyCarTarget = nil,
        rapidFireTimer = 0, lastAfkTime = 0, lastEssencePick = 0,
        armasDetectadas = {}, carClone = nil, moneyLoop = nil, jobLoop = nil,
    }
    ...
    local function isAdmin(player)
        ...
        for _, kw in ipairs(keywords) do if name:find(kw) then return true end end
        return false
    end
    ...
    -- ============================================================
    -- CARREGAMENTO SEGURO DA BIBLIOTECA KAVO
    -- ============================================================
    local Library
    ...
    if not success or not Library then
        warn("[S4ZX] Falha ao carregar a interface Kavo. Verifique sua internet ou executor.")
        return
    end
    ...
    do
        ...
        local aimPartDd = Aim:NewDropdown("Parte do Corpo", "", {"Cabeça", "Peito", "Perna", "Braço"}, function(v) state.aimPart = v end)
        aimPartDd:Set("Cabeça")
        local aimPriorityDd = Aim:NewDropdown("Prioridade", "", {"Distância", "Saúde", "Visibilidade", "Nome Específico"}, function(v) state.aimPriority = v end)
        ...
        Silent:NewSlider("Predição Amount x0.001", "", 1000, 165, 165, function(v) state.saPredictionAmount = v/1000 end)
        Silent:NewDropdown("Método Silent", "", {"Raycast", "FindPartOnRay", "FindPartOnRayWithWhitelist", "FindPartOnRayWithIgnoreList", "Mouse.Hit/Target"}, function(v) state.saMethod = v end)
        Silent:NewDropdown("Parte Alvo (Silent)", "", {"Head", "HumanoidRootPart", "Random"}, function(v) state.saTargetPart = v end)
    ...
    do
        ...
        ESP:NewButton("Cor Esqueleto (Aleatória)", "", function() state.skeletonColor = Color3.fromRGB(math.random(255), math.random(255), math.random(255)) end)
        ESP:NewButton("Cor Box (Aleatória)", "", function() state.boxColor = Color3.fromRGB(math.random(255), math.random(255), math.random(255)) end)
        ESP:NewButton("Cor Ícone de Fala (Aleatória)", "", function() state.talkColor = Color3.fromRGB(math.random(255), math.random(255), math.random(255)) end)
        ...
        PlayersList:NewButton("🔄 Puxar", "", function()
            local name = playerDd:Get(); if not name then return end
    ...
    do
        ...
        Vehicles:NewButton("Destrancar Veículo", "", function()
            for _, v in ipairs(Workspace:GetDescendants()) do if v:IsA("VehicleSeat") then v:SetAttribute("Locked", false); v.Locked = false end end
        end)
        Vehicles:NewButton("Trancar Veículo", "", function()
            for _, v in ipairs(Workspace:GetDescendants()) do if v:IsA("VehicleSeat") then v:SetAttribute("Locked", true); v.Locked = true end end
        end)
        Vehicles:NewButton("Marcar Waypoint", "", function()
            local char = LocalPlayer.Character; if char and char:FindFirstChild("HumanoidRootPart") then state.waypoint = char.HumanoidRootPart.Position end
        end)
        Vehicles:NewButton("Teleportar Waypoint", "", function()
            if state.waypoint then
                local char = LocalPlayer.Character
                if char and char:FindFirstChild("HumanoidRootPart") then char.HumanoidRootPart.CFrame = CFrame.new(state.waypoint + Vector3.new(0, 2, 0)) end
            end
        end)
        Vehicles:NewToggle("Super Velocidade no Carro", "", function(v) state.superCarSpeed = v end)
        Vehicles:NewSlider("Velocidade Super Carro", "", 500, 50, 100, function(v) state.superCarSpeedValue = v end)
        Vehicles:NewToggle("Clonar Carro", "", function(v)
            state.cloneCar = v
            if v then
                local char = LocalPlayer.Character; if not char then return end
                local seat = nil
                for _, obj in ipairs(Workspace:GetDescendants()) do if obj:IsA("VehicleSeat") and obj.Occupant == char.Humanoid then seat = obj; break end end
                if seat then
    ...
    do
        ...
        Visual:NewButton("Cor Box (Verde)", "", function() state.boxColor = Color3.fromRGB(0,255,0) end)
        Visual:NewButton("Cor Esqueleto (Rosa)", "", function() state.skeletonColor = Color3.fromRGB(255,105,180) end)
    ...
    do
        ...
        Movement:NewToggle("Fly Avançado (WASD/E/Q)", "", function(v) state.fly = v; if not v then state.flyStartY = nil end end)
    ...
    do
        ...
        Weapons:NewButton("🔄 Atualizar Armas Detectadas", "", function() updateFakeWeapons() end)
        ...
        local function detectWeapons()
            local list = {}
            for _, obj in ipairs(Workspace:GetDescendants()) do if obj:IsA("Tool") and obj.Name ~= "" then table.insert(list, obj.Name) end end
            for _, c in ipairs({LocalPlayer.Backpack, LocalPlayer.Character}) do if c then for _, obj in ipairs(c:GetChildren()) do if obj:IsA("Tool") then table.insert(list, obj.Name) end end end end
            local unique = {}; local result = {}
            for _, n in ipairs(list) do if not unique[n] then unique[n]=true; table.insert(result,n) end end
            return result
        end
        function updateFakeWeapons()
            local weps = detectWeapons(); fakeWeaponDd:Refresh(weps); if #weps > 0 then fakeWeaponDd:Set(weps[1]) end
        end
        updateFakeWeapons()
        FakeWeapons:NewButton("Criar Fake", "", function()
            local name = fakeWeaponDd:Get(); if not name then return end
            local tool = Instance.new("Tool"); tool.Name = name .. "_Fake"; tool.RequiresHandle = true; tool.CanBeDropped = false; tool.Parent = LocalPlayer.Backpack
            local handle = Instance.new("Part"); handle.Name = "Handle"; handle.Size = Vector3.new(0.5,0.1,0.2); handle.Color = Color3.fromRGB(200,200,200); handle.Material = Enum.Material.SmoothPlastic; handle.Anchored = false; handle.CanCollide = false; handle.Locked = true; handle.Transparency = 0.2; handle.Parent = tool
            local glow = Instance.new("SelectionBox"); glow.Adornee = handle; glow.Color3 = Color3.fromRGB(0,255,255); glow.LineThickness = 0.1; glow.Transparency = 0.5; glow.Parent = handle
    ...
    do
        ...
        Extras:NewToggle("🎙️ Microfone Global", "", function(v)
            ...
            if v then
                if VoiceChatService and VoiceChatService:IsEnabled() then
                    VoiceChatService:SetVoiceEnabled(true); VoiceChatService:SetOutputVolume(100); VoiceChatService:SetInputVolume(100)
                    pcall(function() VoiceChatService:SetSpatialVoiceEnabled(false) end)
                end
            else
                if VoiceChatService then VoiceChatService:SetVoiceEnabled(false); pcall(function() VoiceChatService:SetSpatialVoiceEnabled(true) end) end
            end
        end)
        Extras:NewToggle("Money Hack (Auto)", "", function(v)
            state.moneyHack = v
            if v then
                state.moneyLoop = task.spawn(function() while state.moneyHack do task.wait(0.5) local args={[1]="GiveCash",[2]=state.moneyValue} pcall(function() game:GetService("ReplicatedStorage"):FindFirstChild("Remotes"):FireServer(unpack(args)) end) end end)
            else if state.moneyLoop then task.cancel(state.moneyLoop) end end
        end)
        Extras:NewSlider("Valor do Dinheiro", "", 9999999, 1000, 100000, function(v) state.moneyValue = v end)

        local Jobs = Tab:NewSection("💼 Empregos Detectados")
        local jobsDd = Jobs:NewDropdown("Emprego", "", {}, function() end)
        local function detectJobs()
            local list = {}
            for _, obj in ipairs(Workspace:GetDescendants()) do if obj:IsA("ProximityPrompt") and obj.Enabled then local name = obj.Parent and obj.Parent.Name; if name and name ~= "" then table.insert(list, name) end end end
            local unique = {}; local result = {}
            for _, n in ipairs(list) do if not unique[n] then unique[n]=true; table.insert(result,n) end end
            return result
        end
        local function updateJobs() local j = detectJobs(); jobsDd:Refresh(j); if #j > 0 then jobsDd:Set(j[1]) end end
        updateJobs()
        Jobs:NewButton("🔄 Atualizar Empregos", "", updateJobs)
        Jobs:NewButton("Pegar Emprego", "", function()
            local nome = jobsDd:Get(); if not nome then return end
            state.autoJob = true; state.selectedJob = nome
        ...
        VehicleActions:NewButton("🖐️ PEGAR Veículo (Raycast)", "", function()
            local rayParams = RaycastParams.new(); rayParams.FilterDescendantsInstances = {LocalPlayer.Character}; rayParams.FilterType = Enum.RaycastFilterType.Blacklist
            local result = Workspace:Raycast(Camera.CFrame.Position, Camera.CFrame.LookVector * 100, rayParams)
            if result then
                local hit = result.Instance; local car = hit:FindFirstAncestorOfClass("Model")
                if car and (car:FindFirstChildWhichIsA("VehicleSeat") or car:FindFirstChildWhichIsA("Seat")) then
                    if state.grabbedVehicle then pcall(function() if state.vehicleAlign then state.vehicleAlign:Destroy() end if state.vehicleVel then state.vehicleVel:Destroy() end if state.vehicleGyro then state.vehicleGyro:Destroy() end end); state.grabbedVehicle = nil end
                    state.grabbedVehicle = car
                    local primary = car:FindFirstChild("PrimaryPart") or car:FindFirstChildWhichIsA("BasePart")
                    if primary then
                        state.vehicleAlign = Instance.new("AlignPosition"); state.vehicleAlign.MaxForce = 9999999; state.vehicleAlign.Responsiveness = 200
                        state.vehicleAlign.Attachment0 = primary:FindFirstChild("AlignAttachment") or Instance.new("Attachment", primary)
                        local char = LocalPlayer.Character
                        if char and char:FindFirstChild("HumanoidRootPart") then
                            local root = char.HumanoidRootPart; local attach = root:FindFirstChild("GrabAttach") or Instance.new("Attachment", root); attach.Name = "GrabAttach"
                            state.vehicleAlign.Attachment1 = attach
                        end
                        state.vehicleAlign.Parent = primary
                        state.vehicleVel = Instance.new("LinearVelocity"); state.vehicleVel.MaxForce = 9999999; state.vehicleVel.VelocityConstraintMode = Enum.VelocityConstraintMode.Line
                        state.vehicleVel.Attachment0 = primary:FindFirstChild("VelAttachment") or Instance.new("Attachment", primary); state.vehicleVel.Parent = primary
                        state.vehicleGyro = Instance.new("AngularVelocity"); state.vehicleGyro.MaxTorque = 9999999; state.vehicleGyro.AngularVelocity = Vector3.new(0,0,0)
                        state.vehicleGyro.Attachment0 = primary:FindFirstChild("GyroAttachment") or Instance.new("Attachment", primary); state.vehicleGyro.Parent = primary
                    end
                end
            end
        end)
        VehicleActions:NewButton("💥 TACAR Veículo Segurado", "", function()
            ...
            if primary then
                pcall(function() if state.vehicleAlign then state.vehicleAlign:Destroy() end if state.vehicleVel then state.vehicleVel:Destroy() end if state.vehicleGyro then state.vehicleGyro:Destroy() end end)
                local throwDir = Camera.CFrame.LookVector * 300 + Vector3.new(0, 50, 0)
                pcall(function() primary:ApplyImpulse(throwDir * primary:GetMass()); local randomTorque = Vector3.new(math.random(-5000,5000), math.random(-5000,5000), math.random(-5000,5000)); primary:ApplyAngularImpulse(randomTorque * primary:GetMass() * 0.1) end)
            end
    ...
    do
        ...
        Config:NewToggle("Modo Streamer", "", function(v) state.streamerMode = v; if v then Library:ToggleUI() end end)
        Config:NewToggle("Anti Live", "", function(v) state.antiLive = v end)
        Config:NewToggle("Atalhos Rápidos (CTRL+1 a 0)", "", function(v) state.shortcutsEnabled = v end)
        Config:NewKeybind("Atalho de Ocultar Menu (PC)", "RightShift", Enum.KeyCode.RightShift, function() Library:ToggleUI() end)
    end
    ...
    task.delay(1, function()
        local staffGui = Instance.new("ScreenGui", game.CoreGui); staffGui.Name = "StaffCounter"; staffGui.ResetOnSpawn = false
        local staffFrame = Instance.new("TextLabel", staffGui)
        staffFrame.Size = UDim2.new(0, 120, 0, 30); staffFrame.Position = UDim2.new(0.85, -60, 0.05, 0)
        staffFrame.BackgroundColor3 = Color3.new(0,0,0); staffFrame.BackgroundTransparency = 0.5
        staffFrame.Text = "👑 Staff: 0"; staffFrame.TextColor3 = Color3.new(0,1,0); staffFrame.Font = Enum.Font.GothamBold; staffFrame.TextSize = 14
        Instance.new("UICorner", staffFrame).CornerRadius = UDim.new(0,4)
        task.spawn(function() while true do task.wait(1) local count = 0; for _, p in ipairs(Players:GetPlayers()) do if isAdmin(p) then count = count + 1 end end; staffFrame.Text = "👑 Staff: " .. count end end)
    end)
        ...
        if input.UserInputType == Enum.UserInputType.Keyboard then
            if state.shortcutsEnabled and input.KeyCode >= Enum.KeyCode.One and input.KeyCode <= Enum.KeyCode.Zero then
                ...
                if ctrl then
                    ...
                    if map[input.KeyCode] then map[input.KeyCode]() end
                end
            end
        end
    end)
    ...
    local function createESPObject(kind, props)
        if useDrawing then
            local obj = Drawing.new(kind); obj.Visible = true
            for k, v in pairs(props) do obj[k] = v end
            table.insert(espObjects, obj); return obj
        end
        return nil
    end
    ...
    task.spawn(function()
        while true do
            ...
            if state.espNPCs then
                for _, obj in ipairs(Workspace:GetDescendants()) do
                    if obj:IsA("Model") and obj:FindFirstChild("Humanoid") and not Players:GetPlayerFromCharacter(obj) then
                        if obj:FindFirstChild("HumanoidRootPart") and obj.Humanoid.Health > 0 then table.insert(targets, {player = obj, char = obj, isNPC = true}) end
                    end
                end
            end
            ...
            if state.espAdminList then
                local adminNames = {}
                for _, t in ipairs(targets) do if not t.isNPC and isAdmin(t.player) then table.insert(adminNames, t.player.Name) end end
                if #adminNames > 0 then createESPObject("Text", { Position = Vector2.new(10, 10), Text = "Admins: " .. table.concat(adminNames, ", "), Color = Color3.new(1,0,0), Size = 16, Center = false }) end
            end

            for _, target in ipairs(targets) do
                local char = target.char; local root = char:FindFirstChild("HumanoidRootPart"); local head = char:FindFirstChild("Head"); local hum = char:FindFirstChild("Humanoid")
                ...
                local weaponName = ""; if state.espWeapons then local tool = char:FindFirstChildWhichIsA("Tool"); if tool and tool.Name ~= "" then weaponName = tool.Name end end
                ...
                if state.espBox and headOn and feetOn then
                    local bodyHeight = math.abs(headPos2D.Y - feetPos2D.Y); local bodyWidth = bodyHeight * 0.45; local centerX = (headPos2D.X + feetPos2D.X) / 2
                ...
                if state.espNames and headOn then createESPObject("Text", { Position = Vector2.new(headPos2D.X - 50, headPos2D.Y - 22), Text = target.player.Name or "NPC", Color = color, Size = state.textSize, Center = true }) end
                if state.espWeapons and weaponName ~= "" and headOn then createESPObject("Text", { Position = Vector2.new(headPos2D.X - 50, headPos2D.Y + 30), Text = weaponName, Color = Color3.new(1,1,0), Size = 12, Center = true }) end
                if state.espDistance and myRoot and headOn then createESPObject("Text", { Position = Vector2.new(headPos2D.X - 50, headPos2D.Y + 15), Text = math.floor(dist) .. "m", Color = Color3.new(1,1,1), Size = 12, Center = true }) end
                if state.espLines and rootOn then createESPObject("Line", { From = Vector2.new(screenSize.X / 2, screenSize.Y), To = rootPos2D, Color = color, Thickness = 1 }) end
                if state.espTalking and headOn then createESPObject("Text", { Position = Vector2.new(headPos2D.X + 15, headPos2D.Y - 10), Text = "🗣️", Color = state.talkColor, Size = 12, Center = true }) end
                if state.espEnemyAim and not target.isNPC then
                    ...
                    if playerHead then
                        ...
                        if onScreen then local head2D, _ = worldToScreen(playerHead.Position); createESPObject("Line", { From = head2D, To = aimEnd2D, Color = Color3.fromRGB(255, 100, 100), Thickness = 1 }) end
                    end
                end
            end

            if state.fovCircle then
                if not fovCircleObj then fovCircleObj = Drawing.new("Circle"); fovCircleObj.Visible = true; fovCircleObj.Thickness = 2; fovCircleObj.Filled = false end
                fovCircleObj.Position = screenSize / 2; fovCircleObj.Radius = state.fovRadius; fovCircleObj.Visible = true
                if state.fovRainbow then fovCircleObj.Color = Color3.fromHSV(tick() % 1, 1, 1) else fovCircleObj.Color = Color3.new(1,1,1) end
            elseif fovCircleObj then fovCircleObj.Visible = false end
        end
    end)
    ...
    task.spawn(function()
        while true do
            task.wait(0.03)
            if not state.linhaDeMira then if laserLine then if useDrawing then laserLine:Remove() else laserLine:Destroy() end; laserLine = nil end; continue end
            local char = LocalPlayer.Character; if not char then continue end
            local tool = char:FindFirstChildWhichIsA("Tool"); if not tool then continue end
            local handle = tool:FindFirstChild("Handle"); if not handle then continue end
            local origin = handle.Position; local direction = Camera.CFrame.LookVector * 500
            local params = RaycastParams.new(); params.FilterDescendantsInstances = {char}; params.FilterType = Enum.RaycastFilterType.Blacklist
            local result = Workspace:Raycast(origin, direction, params); local targetPos = result and result.Position or (origin + direction)
            if useDrawing then
    ...
    local function getAimPart(char, partName)
        local map = { Cabeça = "Head", Peito = "UpperTorso", Perna = "LeftLowerLeg", Braço = "RightLowerArm" }
    ...
    task.spawn(function()
        while true do
            ...
            if state.aimbot then
                local best, bestScore = nil, math.huge; local center = Camera.ViewportSize / 2
                for _, p in ipairs(Players:GetPlayers()) do
                    if p == LocalPlayer then continue end
                    ...
                    if state.wallCheck then
                        local head = chr:FindFirstChild("Head"); if head then
                            local params = RaycastParams.new(); params.FilterDescendantsInstances = {LocalPlayer.Character}; params.FilterType = Enum.RaycastFilterType.Blacklist
                            if Workspace:Raycast(Camera.CFrame.Position, (head.Position - Camera.CFrame.Position).Unit * 1000, params) then continue end
                        end
                    end
                    ...
                    elseif state.aimPriority == "Visibilidade" then
                        local head = chr:FindFirstChild("Head"); score = (head and #Camera:GetPartsObscuringTarget({head.Position, LocalPlayer.Character}, {LocalPlayer.Character}) > 0) and 1 or 0
                    elseif state.aimPriority == "Nome Específico" and state.priorityName ~= "" then score = p.Name:lower():find(state.priorityName:lower()) and 0 or 1
                    end
                    if score < bestScore then bestScore = score; best = chr end
                end
                if best then
                    local aimPart = getAimPart(best, state.aimPart); if not aimPart then continue end
                    local targetPos = aimPart.Position
                    if state.aimbotLead and aimPart.Velocity then targetPos = targetPos + aimPart.Velocity * state.leadMultiplier * 0.05 end
                    local alpha = 0.02 + (state.aimForce-1)*0.245; local newCF = CFrame.new(Camera.CFrame.Position, targetPos)
            ...
            while true do
                ...
                if state.silentAim then
                    ...
                    if state.saShowTarget then
                        local target = getClosestPlayerSA(); if target then
    ...
    local oldNamecall = hookmetamethod(game, "__namecall", newcclosure(function(...)
        ...
        if state.silentAim and self == workspace and not checkcaller() and math.random(1,100) <= state.saHitChance then
            ...
            if hitPart then
                if method == "FindPartOnRayWithIgnoreList" and state.saMethod == method then args[2] = Ray.new(args[2].Origin, (hitPart.Position - args[2].Origin).Unit * 1000); return oldNamecall(unpack(args))
                elseif method == "FindPartOnRayWithWhitelist" and state.saMethod == method then args[2] = Ray.new(args[2].Origin, (hitPart.Position - args[2].Origin).Unit * 1000); return oldNamecall(unpack(args))
                elseif method == "FindPartOnRay" and state.saMethod:lower() == method:lower() then args[2] = Ray.new(args[2].Origin, (hitPart.Position - args[2].Origin).Unit * 1000); return oldNamecall(unpack(args))
                elseif method == "Raycast" and state.saMethod == method then args[3] = (hitPart.Position - args[2]).Unit * 1000; return oldNamecall(unpack(args))
                end
            end
        end
        return oldNamecall(...)
    ...
    UserInputService.JumpRequest:Connect(function()
        if state.infJump and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") then LocalPlayer.Character.Humanoid:ChangeState(Enum.HumanoidStateType.Jumping) end
    end)
    ...
    task.spawn(function() while true do task.wait()
        if state.fly then
            ...
            local root = char.HumanoidRootPart; local hum = char:FindFirstChild("Humanoid"); if hum then hum.PlatformStand = true end
    ...
    task.spawn(function() while true do task.wait()
        if state.speedHack then
            local char = LocalPlayer.Character; if not char or not char:FindFirstChild("HumanoidRootPart") then continue end
            local hum = char:FindFirstChild("Humanoid"); if hum and hum.MoveDirection.Magnitude > 0 then
    ...
    task.spawn(function() while true do task.wait(0.3) if state.ghostMode and LocalPlayer.Character then for _, part in ipairs(LocalPlayer.Character:GetDescendants()) do if part:IsA("BasePart") then part.Transparency = 0.85 end end end end end)
    task.spawn(function() while true do task.wait(0.1) if state.derrubarPlayer then for _, p in ipairs(Players:GetPlayers()) do if p ~= LocalPlayer and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then p.Character.HumanoidRootPart.Velocity = Vector3.new(0, -50, 0) end end end end end)
    ...
    task.spawn(function() while true do task.wait(0.1)
        ...
        for _, part in ipairs(Workspace:GetDescendants()) do
            if part:IsA("BasePart") and part.Transparency < 0.9 then
                ...
                for _, kw in ipairs(keywords) do if name:find(kw) then local d = (part.Position - root.Position).Magnitude; if d < nearestDist then nearestDist = d; trash = part end end end
            end
        end
        if trash then
    ...
    task.spawn(function() while true do task.wait(0.5)
        ...
        local char = LocalPlayer.Character; if not char then continue end
        local tool = char:FindFirstChild("Micha") or LocalPlayer.Backpack:FindFirstChild("Micha"); if tool and tool:IsA("Tool") then tool.Parent = char; pcall(function() tool:Activate() end) end
        for _, prompt in ipairs(Workspace:GetDescendants()) do if prompt:IsA("ProximityPrompt") and (prompt.ObjectText:lower():find("micha") or prompt.ActionText:lower():find("roubar")) then pcall(function() fireproximityprompt(prompt) end) end end
    end end)

    -- Fly Car
    task.spawn(function() while true do task.wait(0.05)
        if not state.flyCar then if state.flyCarBV then state.flyCarBV:Destroy(); state.flyCarBV = nil end if state.flyCarBG then state.flyCarBG:Destroy(); state.flyCarBG = nil end state.flyCarTarget = nil; continue end
        local char = LocalPlayer.Character; if not char or not char:FindFirstChild("HumanoidRootPart") then continue end
        if not state.flyCarTarget or not state.flyCarTarget.Parent then
            local nearest, nearestDist = nil, math.huge
            for _, obj in ipairs(Workspace:GetDescendants()) do
                if obj:IsA("VehicleSeat") or obj:IsA("Seat") then local car = obj:FindFirstAncestorOfClass("Model")
                    if car then local p = car:FindFirstChild("PrimaryPart") or car:FindFirstChildWhichIsA("BasePart")
        ...
        local primary = state.flyCarTarget:FindFirstChild("PrimaryPart") or state.flyCarTarget:FindFirstChildWhichIsA("BasePart"); if not primary then continue end
    ...
    task.spawn(function() while true do task.wait(0.1)
        if state.superCarSpeed and LocalPlayer.Character then
            local seat = nil; for _, v in ipairs(Workspace:GetDescendants()) do if v:IsA("VehicleSeat") and v.Occupant == LocalPlayer.Character.Humanoid then seat = v; break end end
            if seat then local car = seat:FindFirstAncestorOfClass("Model"); if car then local p = car:FindFirstChild("PrimaryPart") or car:FindFirstChildWhichIsA("BasePart"); if p then p.Velocity = p.CFrame.LookVector * state.superCarSpeedValue end end end
        end
    end end)

    -- Armas
    task.spawn(function() while true do task.wait(0.5)
        if state.reach then local tool = LocalPlayer.Character and LocalPlayer.Character:FindFirstChildWhichIsA("Tool"); if tool then tool.MaxActivationDistance = state.reachDist end end
        if state.infiniteAmmo then local tool = LocalPlayer.Character and LocalPlayer.Character:FindFirstChildWhichIsA("Tool"); if tool then local ammo = tool:FindFirstChild("Ammo") or tool:FindFirstChild("Bullets") or tool:FindFirstChild("Magazine"); if ammo and ammo:IsA("IntValue") then ammo.Value = 99 end end end
        if state.autoReload then local tool = LocalPlayer.Character and LocalPlayer.Character:FindFirstChildWhichIsA("Tool"); if tool then local ammo = tool:FindFirstChild("Ammo") or tool:FindFirstChild("Bullets"); if ammo and ammo:IsA("IntValue") and ammo.Value == 0 then pcall(function() tool:Reload() end) end end end
        if state.noRecoil then local tool = LocalPlayer.Character and LocalPlayer.Character:FindFirstChildWhichIsA("Tool"); if tool then for _, obj in ipairs(tool:GetDescendants()) do if obj:IsA("SpringConstraint") or obj:IsA("RocketPropulsion") then obj.Enabled = false end end end end
        if state.oneShot then local tool = LocalPlayer.Character and LocalPlayer.Character:FindFirstChildWhichIsA("Tool"); if tool then for _, v in ipairs(tool:GetDescendants()) do if v.Name == "Damage" and v:IsA("NumberValue") then v.Value = 9999 end end end end
        if state.roubarP1 then for _, p in ipairs(Players:GetPlayers()) do if p ~= LocalPlayer and p.Character then local tool = p.Character:FindFirstChildWhichIsA("Tool"); if tool then tool.Parent = LocalPlayer.Backpack end end end end
        if state.roubarP2 then for _, p in ipairs(Players:GetPlayers()) do if p ~= LocalPlayer and p.Character then for _, child in ipairs(p.Character:GetChildren()) do if child:IsA("Tool") then child.Parent = LocalPlayer.Backpack end end end end end
    end end)

    -- Rapid Fire
    task.spawn(function() while true do task.wait()
        if state.rapidFire and UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) then
            local tool = LocalPlayer.Character and LocalPlayer.Character:FindFirstChildWhichIsA("Tool"); if tool and tick() - state.rapidFireTimer >= state.rapidFireDelay then pcall(function() tool:Activate() end); state.rapidFireTimer = tick() end
        end
    end end)

    -- RGB e tamanho
    task.spawn(function() while true do task.wait(0.05) if state.armaColorida then local tool = LocalPlayer.Character and LocalPlayer.Character:FindFirstChildWhichIsA("Tool"); if tool and tool:FindFirstChild("Handle") then tool.Handle.Color = Color3.fromHSV((tick() * state.rgbSpeed) % 1, 1, 1) end end end end)
    task.spawn(function() while true do task.wait(0.5) local tool = LocalPlayer.Character and LocalPlayer.Character:FindFirstChildWhichIsA("Tool"); if tool then pcall(function() tool:ScaleTo(state.armaSize) end) end end end)

    -- God, Stun, Fire, Respawn
    task.spawn(function() while true do task.wait(0.5)
        if state.godMode and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") then local hum = LocalPlayer.Character.Humanoid; hum.MaxHealth = 1e9; hum.Health = 1e9 end
        if state.antiStun and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") then local hum = LocalPlayer.Character.Humanoid; hum:SetStateEnabled(Enum.HumanoidStateType.Physics, false); hum:SetStateEnabled(Enum.HumanoidStateType.FallingDown, false) end
        if state.antiFire and LocalPlayer.Character then for _, part in ipairs(LocalPlayer.Character:GetDescendants()) do if part:IsA("BasePart") and part.Material == Enum.Material.Fire then part.Material = Enum.Material.SmoothPlastic end end end
        if state.autoRespawn and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") and LocalPlayer.Character.Humanoid.Health <= 0 then pcall(function() LocalPlayer:LoadCharacter() end) end
    end end)

    -- Anti AFK
    task.spawn(function() while true do task.wait(1)
        if state.antiAfk and tick() - state.lastAfkTime > 120 then state.lastAfkTime = tick(); local char = LocalPlayer.Character; if char and char:FindFirstChild("HumanoidRootPart") then char.HumanoidRootPart.CFrame = char.HumanoidRootPart.CFrame * CFrame.new(0, 1, 0) end end
    end end)
    ...
    script.Destroying:Connect(function()
        if state.flyCarBV then state.flyCarBV:Destroy() end if state.flyCarBG then state.flyCarBG:Destroy() end
        if state.vehicleAlign then state.vehicleAlign:Destroy() end if state.vehicleVel then state.vehicleVel:Destroy() end if state.vehicleGyro then state.vehicleGyro:Destroy() end
        ...
        local c = LocalPlayer.Character; if c and c:FindFirstChild("Humanoid") then c.Humanoid.PlatformStand = false; c.Humanoid.WalkSpeed = 16 end
    end)
